...
//        System.out.println("日落时间：" + str2);
        ...
        double latitude = 30.625124; // 地球上的纬度（以度为单位）
        double longitude = -104.066507; // 地球上的经度（以度为单位）
        ...
        // 计算太阳角度
        double angle = calculateSunAngle(latitude, longitude, hour, minute);

        System.out.println("太阳角度：" + angle + " 度");

    }
    // 计算太阳角度的方法
    private static double calculateSunAngle(double latitude, double longitude, int hour, int minute) {
        // 计算一年中的日数
        Calendar calendar = Calendar.getInstance();
        ...
        double timeCorrection = 4 * (longitude + 15 * (hour + minute / 60.0) - 180);        double solarTime = hour * 60 + minute + timeCorrection;
        double hourAngle = (solarTime / 4) - 180;
        double zenithAngle = Math.toDegrees(Math.acos(Math.sin(Math.toRadians(latitude)) * Math.sin(Math.toRadians(declination)) + Math.cos(Math.toRadians(latitude)) * Math.cos(Math.toRadians(declination)) * Math.cos(Math.toRadians(hourAngle))));
